...
function GainLossChart() {
  ...
  const chartAccessibilityLabel = `Gain/Loss chart showing price changes. Current value: ${priceFormatter(data[data.length - 1])}`;

  const scrubberAccessibilityLabel = useCallback(
    (index: number) => {
      const value = data[index];
      const status = value >= 0 ? 'gain' : 'loss';
      return `Position ${index + 1} of ${data.length}: ${priceFormatter(value)} ${status}`;
    },
    [data, priceFormatter],
  );
