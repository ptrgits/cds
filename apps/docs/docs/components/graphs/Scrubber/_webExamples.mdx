...
Scrubber can be used to provide horizontal interaction with a chart. As your mouse hovers over the chart, you will see a reference line and scrubber beacon following your cursor.
...
Setting the `label` prop for a series will display a label above the scrubber beacon.
...
Setting the `pulse` prop will cause the scrubber beacons to pulse when the user is not actively scrubbing.
