...
function PositioningExample() {
  ...
  return (
    <Tour
      ...
      onChange={setActiveTourStep}
      steps={tourSteps}
      // Increase distance between coachmark and target
      tourStepOffset={32}
      // Add padding when shifting to stay in viewport
...
Always provide accessibility labels for close buttons and ensure coachmarks are navigable.
