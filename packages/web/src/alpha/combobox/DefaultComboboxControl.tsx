...
>(
  props: ComboboxControlProps<Type, SelectOptionValue> & { ref?: React.Ref<HTMLElement> },
) => React.ReactElement;

export const DefaultComboboxControlComponent = memo(
  forwardRef(
      ...
      const chipsNode = useMemo(() => {
        ...
        const valuesToShow =
          value.length <= maxSelectedOptionsToShow
            ? value
            : value.slice(0, maxSelectedOptionsToShow);
...
export const DefaultComboboxControl =
  DefaultComboboxControlComponent as DefaultSelectControlComponent;
